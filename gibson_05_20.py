...
class gibsondataset(torch.utils.data.Dataset):
    def __init__(self):
        all_points = np.load("train_data/sampled_points.npy")
        all_speeds = np.load("train_data/speed.npy")
        self.data = torch.cat((torch.tensor(all_points, dtype=torch.float32), torch.tensor(all_speeds, dtype=torch.float32)), dim=1)
...
class NN(torch.nn.Module):
    ...
    def encoder_out(self, coords):
        ...
        return encoder_output, coords
    
    def symmetric_op_out(self, encoder_output, coords):
        size = coords.shape[0]
        x0 = encoder_output[:size,...]
        x1 = encoder_output[size:,...]
        ...
        sym_output = torch.cat((x_0, x_1),1)
        return sym_output, coords
        
    def generator_out(self, sym_output, coords):
        x = sym_output
        for ii in range(self.nl2):
            x_tmp = x
            x = self.act(self.generator[ii](x)) 
...
class SubModel(torch.nn.Module):
    ...
    def out(self, x):
        #! This output should be a encoder feature output
        encoder_output, _ = self.network.encoder_out(x)
        # tau, _ = self.network.out(x)
        # window = cosine_window(x[:, 0, None], self.region, self.device)
        x0 = x[:,:self.dim]
        x1 = x[:,self.dim:]        
        x_combined = torch.vstack((x0,x1))
        window = cosine_2d(*self.region, x_combined[:, 0, None], x_combined[:, 1, None])
        window = window.unsqueeze(1)
        windowed_tau = window * encoder_output 
        return windowed_tau, window, encoder_output
...
class Model(torch.nn.Module):
    ...
    def init_network(self):
        ...
        #*: move the generator logic to here
    ...
    def out(self, x):
        ...
        normalized_encoder_output = outputs/ws 
        sym_output, _ = self.network.symmetric_op_out(normalized_encoder_output, x)
        generator_output, _ = self.network.generator_out(sym_output, x)
        return generator_output, x, raws, windows
    ...
    def plot_out(self, x):
        ...
        for i, subnet in enumerate(self.subnets):
            windowed_output, window, raw_output = subnet.out(x)
            windowed_outputs.append(windowed_output)
            windows.append(window)
        windows = torch.stack(windows, dim=0)
        ws  = torch.sum(windows, dim=0)
        subnet_outputs = []
        for i, w_output in enumerate(windowed_outputs, 1):
            subnet_feature =  w_output / ws
            sym_output, _ = self.network.symmetric_op_out(subnet_feature, x)
            generator_output, _ = self.network.generator_out(sym_output, x)
            subnet_outputs.append(generator_output)
        ...
        for epoch in range(1, self.Params['Training']['Number of Epochs']+1):
            ...
            while True:
                ...
                if (diff_ratio < 1.2 and diff_ratio > 0):#1.5
                    ...
                    iter+=1
                    with torch.no_grad():
                        random_number = random.randint(0, 4) if len(prev_state_queue)>4 else 0
    ...
    def plot(self,epoch,total_train_loss,alpha):
        ...
        if True:
            ...
            plt.tight_layout()
            plt.savefig(self.Params['ModelPath']+"/plots"+str(epoch) + "_subnets.png")
            plt.close()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(folder):
        os.makedirs(folder)
    model = Model(folder, datapath, 3, "cuda")
