...
class NN(nn.Module):
    ...
    @staticmethod
    def Loss1(self, points, Yobs, beta):
        ...
        T00 = 4*torch.exp(-4*D_norm)*tau[:,0]**2
        T10 = (1-torch.exp(-4*D_norm))*S0 
        T11 = (1-torch.exp(-4*D_norm))*S1
        T20 = 4*torch.exp(-2*D_norm)*torch.einsum('ij,ij->i', D, DT0)*tau[:,0]/D_norm*(1-torch.exp(-2*D_norm))
        T21 = 4*torch.exp(-2*D_norm)*torch.einsum('ij,ij->i', D, DT1)*tau[:,0]/D_norm*(1-torch.exp(-2*D_norm))
        under0 = T00 + T10 + T20
        under1 = T00 + T11 + T21
        
        Ypred0 = torch.sqrt(under0)
        Ypred1 = torch.sqrt(under1)
        sq_Yobs0 = Yobs[:,0]
        sq_Yobs1 = Yobs[:,1]
        l0 = sq_Yobs0*(Ypred0)
        l1 = sq_Yobs1*(Ypred1)
    ...
    def Loss(self, points, Yobs, beta):
        ...
        S0 = torch.einsum('ij,ij->i', DT0, DT0)
        S1 = torch.einsum('ij,ij->i', DT1, DT1)
        T00 = 100*torch.exp(-20*D_norm)*tau[:,0]**2
        T10 = (1-torch.exp(-20*D_norm))*S0 
        T11 = (1-torch.exp(-20*D_norm))*S1
        T20 = 20*torch.exp(-10*D_norm)*torch.einsum('ij,ij->i', D, DT0)*tau[:,0]/D_norm*(1-torch.exp(-10*D_norm))
        T21 = 20*torch.exp(-10*D_norm)*torch.einsum('ij,ij->i', D, DT1)*tau[:,0]/D_norm*(1-torch.exp(-10*D_norm))
        under0 = T00 + T10 + T20
        under1 = T00 + T11 + T21
    ...
    def TravelTimes(self, Xp):
        ...
        TT = (1-torch.exp(-10*D_norm))*tau[:,0]
    ...
    def Speed1(self, Xp):
        ...
        Ypred = 1/torch.sqrt(under1)

        
        del Xp, tau, dtau, T00, T11, T21, under1
        return Ypred
