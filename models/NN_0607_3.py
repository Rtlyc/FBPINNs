...
class NN(nn.Module):
    ...
    def Loss(self, points, Yobs, beta):
        ...
        T00 = 100*torch.exp(-20*D_norm)
        T10 = S0  
        T11 = S1 
        T20 = 20*torch.exp(-10*D_norm)*torch.einsum('ij,ij->i', D, DT0)/D_norm
        T21 = 20*torch.exp(-10*D_norm)*torch.einsum('ij,ij->i', D, DT1)/D_norm
        Y0 = T00 + T10 + T20 
        Y1 = T00 + T11 + T21

        Ypred0 = torch.sqrt(Y0)
        Ypred1 = torch.sqrt(Y1)
        sq_Yobs0 = Yobs[:,0]
        sq_Yobs1 = Yobs[:,1]
        l0 = sq_Yobs0*(Ypred0)
        l1 = sq_Yobs1*(Ypred1)
